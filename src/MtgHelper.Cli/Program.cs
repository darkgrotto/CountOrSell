...
var dbOptions = new DbContextOptionsBuilder<MtgHelperDbContext>()
...
scryfallClient.DefaultRequestHeaders.Add("User-Agent", "MtgHelper/2.0");
...
mtgJsonClient.DefaultRequestHeaders.Add("User-Agent", "MtgHelper/2.0");
var mtgJson = new MtgJsonService(mtgJsonClient);

var jsonOptions = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };

// ========== ROOT COMMAND ==========
var rootCommand = new RootCommand("MtgHelper CLI - Admin tool for managing MTG card data");
...
publishCommand.SetHandler(async (string outputDir, string? version, string? deltaFrom, string? baseDbPath) =>
{
    ...
    var fullTempDir = Path.Combine(Path.GetTempPath(), $"mtghelper-full-{Guid.NewGuid():N}");
    ...
    if (deltaFrom != null)
    {
        ...
        var deltaTempDir = Path.Combine(Path.GetTempPath(), $"mtghelper-delta-{Guid.NewGuid():N}");
        Directory.CreateDirectory(deltaTempDir);

        try
        {
            // Open old database to find what's changed
            var oldDbOptions = new DbContextOptionsBuilder<MtgHelperDbContext>()
                .UseSqlite($"Data Source={baseDbPath}")
                .Options;
            using var oldDb = new MtgHelperDbContext(oldDbOptions);
...
var migrateSourceOption = new Option<string>("--source", "Path to v1 mtghelper.db file") { IsRequired = true };
...
migrateCommand.SetHandler(async (string source) =>
{
    ...
    var v1Options = new DbContextOptionsBuilder<MtgHelperDbContext>()
...
async Task CreateDataOnlyDb(MtgHelperDbContext sourceDb, string outputPath)
{
